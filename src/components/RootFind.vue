<template>
  <div class="container method">
    ...
    <form v-on:submit.prevent="calculate" class="form-box">
      ...
      <input type="number" step="any" v-model="interval_start" placeholder="Inicio del intervalo" />

      <div class="helper">
        <label>Final del Intervalo</label>
        <p>Introduce el final del intervalo de solución.</p>
      </div>
      <input type="number" step="any" v-model="interval_end" placeholder="Final del intervalo" />
    ...
    calculate() {
      if (this.metodo == "Bisección") {
        if (
          ...
          this.toast.fire({ icon: "error", title: "Datos faltantes." });
        } else {
          try {
            this.bisection(
              this.evaluar,
            ...
            this.toast.fire({ icon: "success", title: "Calculado" });
          } catch (e) {
  ...
  computed: {
    ...
    get_alert_ecuation() {
      try {
        ...
        if (
          (isNaN(valor) || valor == Infinity || valor == Infinity * -1) &&
          this.ecuation !== ""
        ) {
